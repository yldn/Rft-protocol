        ...
        for fd, event in event_list:
            if(fd == socket.fileno()):
                ...
                if(packets_received>9):
                    ...
                    write_thread.write()
                    # mock_missingrange = (500).to_bytes(8,byteorder="big") +(599).to_bytes(8,byteorder="big") 
                    # packet = rft_packet.rft_packet.create_ack_packet(cid,max_ack,rft_packet.ACK,nack_ranges = mock_missingrange)
